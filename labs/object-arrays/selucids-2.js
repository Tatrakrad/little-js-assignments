...
// var result = twoConsortKings(apiResult);
// console.log("kings with 2 consorts", result);

//Write a function that returns all kings with 0 consorts

function noConsortKings(data){
...
// var result = noConsortKings(apiResult);
// console.log("kings sans consort of any sort", result);

//Once those two are complete, refactor so that they can both use the same function

function kingsWithNConsorts(data,num){

  var output = [];

  for (var i = 0; i < data.rulers.length; i++){
    var ruler = data.rulers[i];
    if (ruler.consort.length === num){
      output.push(ruler);
    }
  }
  return output;
}
// var result = kingsWithNConsorts(apiResult,1);
// console.log("kings with n consorts...", result);

//-------------------------

//Write a function that returns the king with the longest name

function longestNamedKing(data){

  var longest = data.rulers[0];
  for (var i = 1; i < data.rulers.length; i++){
    var ruler = data.rulers[i];
    if (ruler.name.length > longest.name.length){
      longest = ruler;
    }
  }
  return longest;
}
var result = longestNamedKing(apiResult);
console.log("King with the LONGEST NAME: ",result);

//Write a function that takes a consorts name and returns all the kings she was consort with

function findKingsByConsort(data,selectedConsort){
  var output = [];
  for (var i = 0; i < data.rulers.length; i++){
    var ruler = data.rulers[i];
    if (ruler.consort.length !== 0){
      for (var j=0; j<ruler.consort.length;j++){
        var consort = ruler.consort[j];
        if (consort === selectedConsort){
          output.push(ruler);
        }
      }
...
var consort1 = "Cleopatra Thea";
var result = findKingsByConsort(apiResult,consort1);
console.log("Kings with Consort ",consort1,result);

//-------------------------

//Write a function that returns all kings with a consort whose name contains "Cleopatra"

function kingsWithConsortCleopatra(data){
  var output = [];
  for (var i = 0; i<data.rulers.length; i++){
    var ruler = data.rulers[i];
    if (ruler.consort.length !== 0){
...
var result = kingsWithConsortCleopatra(apiResult);
console.log("Kings with Consort who name contain Cleopartra ==> ",result);

//Write a function that returns all kings with a consort whose name contains "Laodice"

function kingsWithConsortLaodice(data){
  var output = [];
  for (var i = 0; i<data.rulers.length; i++){
    var ruler = data.rulers[i];
    if (ruler.consort.length !== 0){
...
var result = kingsWithConsortLaodice(apiResult);
console.log("Kings with Consort who name contain Laodice ==> ",result);

//Once those two are complete, refactor so that they can both use the same function

function kingsWithConsortSearch(data,search){
  var output = [];
  for (var i = 0; i<data.rulers.length; i++){
    var ruler = data.rulers[i];
    if (ruler.consort.length !== 0){
...
var searchName = "Selene";
var result = kingsWithConsortSearch(apiResult,searchName);
console.log("Kings with Consort searching for name ==> ",searchName,result);
