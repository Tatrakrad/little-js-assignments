...
function averagePrice(data){
  ...
  output = sumPrices / totalItems;
  return output;
}

var result = averagePrice(items);
console.log("Average Price for items ===> ", result);

// Get all the items that cost between $14.00 and $18.00 USD

function itemsInPriceRange(data,min,max){
  ...
  for(var i=0; i<data.length;i++){
    ...
    var countryCode = item.currency_code;


    if ((itemCost >= min && itemCost <= max) && countryCode.includes("USD")){
      output.push(item);
    }
...
var result = itemsInPriceRange(items,14,18);
// console.log("Items within price range ===> \n",result);

// Log the name and price of every item with a "GBP" currency code and print its name and price.

function etsyCurrencyUK(data){
  ...
  for(var i=0;i<data.length;i++){
    ...
    if(countryCode.includes("GBP")){
      var ukListing = {
        "Name":itemName,
        "Price":itemCost,
      };
      output.push(ukListing);
    }
  }
  return output;
}

var result = etsyCurrencyUK(items);
// console.log("GBP / UK Listings Summary ===> \n", result);

// Return all items that are at least partially made of wood.

function etsyWoodConstruction(data,material){
...
var result = etsyWoodConstruction(items,"wood");
// console.log("Things made at least partially out of wood =====>\n",result);

// Find which items are made of eight or more materials.

function etsyItemsWithNMaterials(data,minMaterials){
  ...
  for(var i=0;i<data.length;i++){
    ...
    var item = data[i];
    if (item.materials.length >= minMaterials){
      output.push(item);
    }
  }
  return output;
}
var result = etsyItemsWithNMaterials(items,8);
// console.log("Items made with 8 or more materials ====> \n", result);

// Return the items that were made by their sellers

function etsyItemsMadeBySeller(data){
...
var result = etsyItemsMadeBySeller(items);
console.log("Items made by Seller ====>\n",result);
