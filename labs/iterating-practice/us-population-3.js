...
function year20kDiff(data) {
  var output = [];

  output = data.filter(function(obj){
    var diff = Math.abs(obj.females - obj.males);
    if (diff>20000){
      return true;
    }
    else {
...
result = year20kDiff(dataset);
console.log("Years where there is a greater than 20k difference between the male and female population:\n",result);

// Write a function that returns all years where there is less than a 100K population. Requirement: use filter.

function year100kTotal(data) {
  var output = [];
...
result = year100kTotal(dataset);
console.log("Years where there is less than 100k population\n",result);
...
function popData20kRemap(data){
  ...
  output = filtered.map(function(popObj){
    var newObj = {
      age: popObj.age,
      males: popObj.males,
      females: popObj.females,
      total: popObj.total
    }
    return newObj;
  });
...
result = popData20kRemap(dataset);
console.log("Pop data remapped for ages and populations with a >20k difference only: \n", result);
...
function popDataSub100k(data){
  ...
  output = filtered.map(function(popObj){
    var newObj = {
      year:popObj.year,
      country:popObj.country,
      age: popObj.age,
      females: popObj.females,
      males: popObj.males,
      total: popObj.total,
      diff: Math.abs(popObj.males - popObj.females)
    }
    return newObj;
  });
return output;
}
result = popDataSub100k(dataset);
console.log("Years with a sub 100k population, including female male differential :\n",result);
